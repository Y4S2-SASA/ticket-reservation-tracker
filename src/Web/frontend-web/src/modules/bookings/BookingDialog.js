...
const BookingDialog = ({ settings, onClose, onSave, callBackData }) => {
  ...
  const checkTrainAvailability = async () => {
    try {
        console.log(formDataDestination[0].id)
        console.log(formDataOrigin)
        console.log(formDataStartDate)
        console.log(formDataselectedPClass)

        setSchedulesLoading(true)
        const stationsRes = await ScheduleAPIService.getScheduleTrainsData(

        )
        setSchedules(stationsRes)
    } catch (error) {
        console.log(error)
        setSchedules([])
    } finally {
